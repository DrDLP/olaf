...
Audio-reactive kawaii guitar player.

Behavior
--------
- Uses stems/guitar.wav to detect when the guitar is active and how strong it is.
...
Sprites (relative to repo root, under assets/sprites):
    guitar_idle.png   -> idle pose (no guitar playing)
    guitar_play1.png  -> playing pose A (normal guitar activity)
    guitar_play2.png  -> playing pose B (normal guitar activity)
    guitar_high.png   -> strong accent pose (when guitar is very loud)

Rules:
- When guitar energy is very low -> stay on guitar_idle.png, with a small tilt
  (left/base/right/base) driven by the rhythm stem.
- When guitar energy is moderate -> alternate guitar_play1.png and
  guitar_play2.png, with a switching speed that depends on the rhythm energy.
- When guitar energy is strong (>= 70% of the normalized maximum) -> use
  guitar_high.png and apply a stronger wobble/tilt driven by the guitar signal.
...
OUT_FPS = 24
OUT_BITRATE = "6M"
OUT_FILENAME = "guitar_dog_480.mp4"
...
# Guitar energy thresholds (on normalized 0..1 envelope)
GTR_IDLE_THR = 0.06   # below -> idle
GTR_STRONG_THR = 0.70 # above -> "high" pose

# Idle tilt parameters (when guitar is silent, driven by rhythm stem)
IDLE_ANGLE_MIN_DEG = 0.2
IDLE_ANGLE_MAX_DEG = 0.5
IDLE_TILT_CYCLE_HZ = 1.5  # pattern L -> base -> R -> base

# Play animation speed (alternation of play1/play2) in Hz
PLAY_BASE_FREQ = 1.5  # minimum alternation speed
PLAY_EXTRA_FREQ = 3.0 # extra speed when rhythm energy is high

# Strong wobble when guitar is loud (on guitar_high pose)
...
GTR_WAV = os.path.join(STEMS_DIR, "guitar.wav")
RYTHM_WAV = os.path.join(STEMS_DIR, "rythm_merged.wav")

SPR_IDLE   = os.path.join(SPRITES_DIR, "guitar_idle.png")
SPR_PLAY1  = os.path.join(SPRITES_DIR, "guitar_play1.png")
SPR_PLAY2  = os.path.join(SPRITES_DIR, "guitar_play2.png")
SPR_HIGH   = os.path.join(SPRITES_DIR, "guitar_high.png")
...
print(f"[guitar_dog] Loading guitar stem: {GTR_WAV}")
y_gtr, sr_gtr, D_gtr = load_audio_normalized(GTR_WAV)

print("[guitar_dog] Computing guitar energy envelope...")
t_gtr_env, e_gtr_env = rms_envelope(y_gtr, sr_gtr)
gtr_energy_at = make_interpolator(t_gtr_env, e_gtr_env)

print(f"[guitar_dog] Loading rhythm stem: {RYTHM_WAV}")
y_r, sr_r, D_r = load_audio_normalized(RYTHM_WAV)

print("[guitar_dog] Computing rhythm envelope...")
...
D = min(D_gtr, D_r)
print(f"[guitar_dog] Clip duration: {D:.2f} seconds")
...
print("[guitar_dog] Loading sprites...")
...
gtr_e_frames = np.array([gtr_energy_at(float(t)) for t in frame_times])
...
for k in range(n_frames):
    ...
    if e <= GTR_IDLE_THR:
        state[k] = 0
    elif e < GTR_STRONG_THR:
        state[k] = 1
    else:
        state[k] = 2

# Idle tilt angles (small wobble based on rhythm energy)
...
# Play animation alternation (play1 / play2) driven by rhythm energy
...
# Strong wobble angles for high state (based on guitar energy)
high_angles = np.zeros(n_frames, dtype=np.float32)
phase_high = 0.0
for k in range(n_frames):
    e_g = gtr_e_frames[k]
    freq = HIGH_WOBBLE_BASE_HZ + HIGH_WOBBLE_EXTRA_HZ * e_g
    # Integrate phase for smooth continuous wobble
    phase_high += freq / OUT_FPS * 2.0 * np.pi
    amp = HIGH_ANGLE_MIN_DEG + (HIGH_ANGLE_MAX_DEG - HIGH_ANGLE_MIN_DEG) * e_g
...
def make_frame(t: float):
    ...
    if st == 0:
        ...
        # Normal play: alternate play1 / play2, no tilt
        sprite_idx = IDX_PLAY1 if play_indices[k] == 0 else IDX_PLAY2
        angle_deg = 0.0
    else:
        # Strong level: high pose with stronger wobble based on guitar
        sprite_idx = IDX_HIGH
    ...
    # Safe clipping in case PW/PH > OUT_WIDTH/OUT_HEIGHT
    x1c = max(0, x1)
    y1c = max(0, y1)
...
print(f"[guitar_dog] Rendering to {OUT_PATH}")
