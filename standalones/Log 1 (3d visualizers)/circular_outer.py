...
OUTPUT_VIDEO_SILENT = "output_circular_hist_silent_outer.mp4"
OUTPUT_VIDEO_WITH_AUDIO = "output_circular_hist_with_audio_outer.mp4"
...
N_BINS = 4000             # number of radial bars around the circle
WAVE_WINDOW_SAMPLES = 4000  # # of waveform samples used per frame (~N_BINS)
...
# energy in [0, 1] is mapped to:
#   energy = 0  -> largest black disk (quiet sections)
#   energy = 1  -> smallest black disk (peaks)
MIN_CIRCLE_RADIUS = 0.35     # radius at highest energy (smallest disk)
MAX_CIRCLE_RADIUS = 1.8      # radius at lowest energy (largest disk)
CIRCLE_ENERGY_EXP = 1.4      # >1 emphasizes strong peaks (stronger shrink)

# ---- Outer ring (bars) geometry ---------------------------------------------
# Fixed outer radius where bars are clamped.
# Bars will always live between [circle_radius, OUTER_RING_RADIUS].
OUTER_RING_RADIUS = 4.0

# Waveform histogram settings (can garder tes valeurs ou ajuster un peu)
MIN_BAR_LENGTH = 0.2         # minimal bar span beyond the circle
BAR_LENGTH_SCALE = 3.0       # how much amplitude increases bar span
BAR_EXPONENT = 0.8           # <1: more mids, >1: only big peaks
...
def update_histogram_geometry(
    ...
    Geometry rules now:
      - The black disk radius depends on RMS energy (inverse mapping).
      - Bars start just outside the black disk (inner radius).
      - Bars are clamped so their tips never go beyond OUTER_RING_RADIUS.
      => From inside to outside: black circle -> bars -> fixed outer circle.
    """
    # ------------------------------------------------------------------
    # 1) Circle radius from energy (inverse mapping)
    # ------------------------------------------------------------------
    e = max(0.0, min(1.0, energy))
    e_shaped = e ** CIRCLE_ENERGY_EXP

    # energy = 0   -> circle_radius = MAX_CIRCLE_RADIUS (largest disk)
    # energy = 1   -> circle_radius = MIN_CIRCLE_RADIUS (smallest disk)
    circle_radius = MAX_CIRCLE_RADIUS - (MAX_CIRCLE_RADIUS - MIN_CIRCLE_RADIUS) * e_shaped

    # Maximum possible span for a bar (from smallest disk to outer ring)
    max_bar_span = max(0.0, OUTER_RING_RADIUS - MIN_CIRCLE_RADIUS)
    ...
    # ------------------------------------------------------------------
    # 3) Amplitude -> bar span (how far it goes beyond the circle)
    # ------------------------------------------------------------------
    amp = np.abs(window_bins)
    amp_norm = np.clip(amp / amp_ref, 0.0, 1.0)

    # Optionally reduce the influence of global loudness so that
    # "energy peaks" don't blow up the outer envelope of the histogram.
    amp_norm = amp_norm / (0.3 + 0.7 * e)
    amp_norm = np.clip(amp_norm, 0.0, 1.0)

    amp_shaped = amp_norm ** BAR_EXPONENT

    bar_span = MIN_BAR_LENGTH + BAR_LENGTH_SCALE * amp_shaped
    # Bars must stay between circle_radius and OUTER_RING_RADIUS
    bar_span = np.clip(bar_span, 0.0, max_bar_span)
    ...
    for i in range(N_BINS):
        ...
        # Bars start at the current circle radius
        base_r = circle_radius
        # and extend outward, clamped so they never exceed OUTER_RING_RADIUS
        tip_r = min(circle_radius + bar_span[i], OUTER_RING_RADIUS)

        x0 = base_r * math.cos(angle)
        y0 = base_r * math.sin(angle)
        x1 = tip_r * math.cos(angle)
        y1 = tip_r * math.sin(angle)
    ...
    # Scale the black circle (unit mesh) to the desired radius
