...
OUTPUT_VIDEO_WITH_AUDIO = "output_circular_hist_with_audio_outer_no_border.mp4"
...
N_BINS = 2000             # number of radial bars around the circle
WAVE_WINDOW_SAMPLES = 2000  # # of waveform samples used per frame (~N_BINS)
...
MIN_CIRCLE_RADIUS = 0.15     # radius at highest energy (smallest disk)
...
BAR_LENGTH_SCALE = 3.5       # how much amplitude increases bar span
...
def update_histogram_geometry(
    ...
    Geometry rules now:
      ...
      - Bars start at the black disk radius.
      - Bars extend outward with no hard outer clamp; their length is
        purely driven by the waveform amplitude.
      => From inside to outside: black circle -> bars (unbounded).
    """
    # ------------------------------------------------------------------
    ...
    # Optional shaping: BAR_EXPONENT < 1.0 => mid-levels more visible,
    #                   BAR_EXPONENT > 1.0 => only strong peaks get long.
    ...
    for i in range(N_BINS):
        ...
        # and extend outward freely according to bar_span (no outer clamp)
        tip_r = circle_radius + bar_span[i]
